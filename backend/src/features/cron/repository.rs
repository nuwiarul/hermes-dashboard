...
fn parse_job(j: serde_json::Value) -> CronJobDto {
    ...
    let enabled = j.get("enabled")
        .and_then(|v| v.as_bool())
        .unwrap_or(true);
    
    let state_str = j.get("state")
        .and_then(|v| v.as_str())
        .unwrap_or(if enabled { "scheduled" } else { "paused" })
        .to_string();
    
    let next_run = j.get("next_run_at")
        .and_then(|v| v.as_str())
        .map(|s| s.to_string());
    ...
    let skills: Vec<String> = j.get("skills")
        .and_then(|v| v.as_array())
        .map(|arr| arr.iter()
            .filter_map(|v| v.as_str().map(|s| s.to_string()))
            .collect())
    ...
    let script = j.get("script")
        .and_then(|v| v.as_str())
        .map(|s| s.to_string());
    
    let no_agent = j.get("no_agent")
        .and_then(|v| v.as_bool())
        .unwrap_or(false);
    
    let workdir = j.get("workdir")
        .and_then(|v| v.as_str())
        .map(|s| s.to_string());
    ...
    let last_run = j.get("last_run_at")
        .and_then(|v| v.as_str())
        .map(|s| s.to_string());
