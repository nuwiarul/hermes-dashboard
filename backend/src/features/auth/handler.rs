...
pub async fn refresh(
    ...
    let refresh_token = extract_token_from_cookie(&request, "refresh_token")
        .map_err(|_| {
    ...
    let claims = jwt::validate_refresh_token(&refresh_token, &state.jwt)
        .map_err(|_| {
            (StatusCode::UNAUTHORIZED, Json(serde_json::json!({
                "success": false,
    ...
    let (new_access, new_refresh) = jwt::generate_token_pair(&claims.sub, &state.jwt)
        .map_err(|e| {
...
fn extract_token_from_cookie(request: &Request<axum::body::Body>, name: &str) -> Result<String, StatusCode> {
