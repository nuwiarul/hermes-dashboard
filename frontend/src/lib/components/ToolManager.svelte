...
<div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
    <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-xl font-semibold text-gray-900">Tool Manager</h2>
                <p class="text-sm text-gray-500">Enable or disable agent tools</p>
            </div>
        </div>
        <div class="flex items-center gap-3 text-sm">
            <span class="text-green-600 font-medium">{enabledCount} enabled</span>
            <span class="text-gray-400">·</span>
            <span class="text-red-600 font-medium">{disabledCount} disabled</span>
        </div>
    </div>
            ...
            <div class="mb-6">
                ...
                <div class="space-y-2">
                    {#each items as toolset}
                        <div class="flex items-center justify-between p-3 rounded-lg border {toolset.enabled ? 'border-gray-200 bg-white' : 'border-red-200 bg-red-50'}">
                            <div class="flex-1">
                                <div class="flex items-center gap-2">
                                    <span class="font-medium text-gray-900">{toolset.name}</span>
                                    {#if !toolset.enabled}
                                        <span class="px-2 py-0.5 text-xs font-medium bg-red-100 text-red-800 rounded">Disabled</span>
                                    {/if}
                                </div>
                                <p class="text-sm text-gray-500">{toolset.description}</p>
                            </div>
                            <button
                                onclick={() => toggleToolset(toolset.name, !toolset.enabled)}
                                disabled={toggling === toolset.name}
                                class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors {toolset.enabled ? 'bg-green-600' : 'bg-gray-300'} disabled:opacity-50"
                            >
